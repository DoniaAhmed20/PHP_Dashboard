...
$customers = $customerDb->get_all_record();

$priceListDb = new priceList();
$priceLists = $priceListDb->get_all_record();

$priceListsNames = array();
foreach ($priceLists as $priceList) {
    $priceListsNames[$priceList["id"]] = $priceList["name"];
}
...
foreach ($customers as $customer) {
   ...
   <td>' . (isset($customerNames[$item["customer_id"]]) ? $customerNames[$item["customer_id"]] : '') . '</td>
   <td>' . (isset($priceListsNames[$item["priceList_id"]]) ? $priceListsNames[$item["priceList_id"]] : '') . '</td>
   
     <td>
     </button>
            <button class="btn btn-info">
                <a class="text-decoration-none text-white" href="order_detailes.php?showId=' . $item["id"] . '">
                <i class="fa fa-eye" style="font-size:17px"></i>
                </a>
            </button>
     <button class="btn " style="background-color:#66CDAA"><a class="text-decoration-none text-black"href="user_update.php?updateId=' . $item["id"] . '" ><i class="fa fa-edit "></i></a></button>
